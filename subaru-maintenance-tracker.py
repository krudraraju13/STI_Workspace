...
# =====================================================================
# SYSTEM EMBEDDED DIAGRAM ASSETS (BASE64 VECTOR WRAPPER)
# =====================================================================
HEAD_BOLT_SVG_DATA = """<svg viewBox="0 0 450 220" width="100%" height="auto" style="max-width: 600px; display:block; margin:auto; background-color: var(--secondary-background-color); border-radius: 8px; border: 1px solid rgba(128, 128, 128, 0.2); padding: 12px;">
...
PISTON_RING_SVG_DATA = """<svg viewBox="0 0 400 400" width="100%" height="auto" style="max-width: 500px; display:block; margin:auto; background-color: var(--secondary-background-color); border-radius: 8px; border: 1px solid rgba(128, 128, 128, 0.2); padding: 12px;">
  <!-- Cylinder Bore -->
...
if HAS_STREAMLIT and st.runtime.exists():
  ...
  <line x1="76" y1="68" x2="96" y2="68" stroke="#94a3b8" stroke-width="3"/>
    ...
    # Encode vectors dynamically to support offline high-speed page-renders (v124)
    import base64
    head_bolt_b64_val = base64.b64encode(HEAD_BOLT_SVG_DATA.encode("utf-8")).decode("utf-8")
    piston_ring_b64_val = base64.b64encode(PISTON_RING_SVG_DATA.encode("utf-8")).decode("utf-8")
    ...
    st.markdown(
        ...
        /* Direct button-image-clickable zoom-box system (v124) */
        .st-key-btn_expand_head_bolt button {
            background-image: url("data:image/svg+xml;base64,__HEAD_BOLT_B64__") !important;
        ...
        .st-key-btn_expand_head_bolt button:hover {
            transform: scale(1.02) !important;
            box-shadow: 0 10px 20px rgba(255, 0, 127, 0.15) !important;
            border-color: #FF007F !important;
            background-color: var(--secondary-background-color) !important;
        }
        .st-key-btn_expand_head_bolt button:active {
        ...
        .st-key-btn_expand_piston_ring button {
            background-image: url("data:image/svg+xml;base64,__PISTON_RING_B64__") !important;
            background-size: contain !important;
            background-repeat: no-repeat !important;
            background-position: center !important;
            background-color: var(--secondary-background-color) !important;
        ...
        .st-key-btn_expand_piston_ring button:hover {
            transform: scale(1.02) !important;
            box-shadow: 0 10px 20px rgba(255, 0, 127, 0.15) !important;
            border-color: #FF007F !important;
            background-color: var(--secondary-background-color) !important;
        }
        .st-key-btn_expand_piston_ring button:active {
            transform: scale(0.98) !important;
        }
        .st-key-btn_expand_piston_ring button * {
            display: none !important;
        ...
        """.replace("__HEAD_BOLT_B64__", head_bolt_b64_val).replace("__PISTON_RING_B64__", piston_ring_b64_val),
...
''', unsafe_allow_html=True)
    ...
    with tab_history:
        ...
        if api_url:
            ...
            st.write("If you have offline records saved in your local, use this tool to synchronize them to the Cloud.")
            
            col_merge, col_spacer = st.columns([1, 2])
            with col_merge:
                if st.button("☁ Push Local Items to Cloud", use_container_width=True, help="Scan local file and upload any missing entries to the Cloud"):
    ...
    with tab_manual:
        ...
        with st.expander("☰ Subaru WRX STI Powertrain & Chassis Specifications"):
            st.markdown(
                """
                ### ☰ Official 2016 Subaru WRX STI Specifications
